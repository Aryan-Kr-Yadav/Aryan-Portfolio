...
//#endregion
//#region node_modules/react-activity-calendar/build/chunks/index-CLOTiFb2.js
...
var query$1 = "(prefers-color-scheme: dark)";
function useColorScheme() {
	const [colorScheme, setColorScheme] = (0, import_react.useState)(() => typeof window === "undefined" ? "light" : window.matchMedia(query$1).matches ? "dark" : "light");
...
var loadingAnimationName = `${NAMESPACE}--loading-animation`;
...
function usePrefersReducedMotion() {
	const [prefersReducedMotion, setPrefersReducedMotion] = (0, import_react.useState)(() => typeof window === "undefined" ? true : window.matchMedia(query).matches);
	(0, import_react.useEffect)(() => {
...
function validateActivities(activities, { minLevel, maxLevel }) {
	if (activities.length === 0) throw new Error("Activity data must not be empty.");
	for (const { date, level } of activities) {
		if (!isValid(parseISO(date))) throw new Error(`Activity date '${date}' is not a valid ISO 8601 date string.`);
		if (level < minLevel || level > maxLevel) throw new RangeError(`Activity level ${level} for ${date} is out of range. It must be between ${minLevel} and ${maxLevel}.`);
	}
}
function groupByWeeks(activities, weekStart = 0) {
	...
	return range(Math.ceil(paddedActivities.length / 7)).map((weekIndex) => paddedActivities.slice(weekIndex * 7, weekIndex * 7 + 7));
}
/**
...
function range(fromArg, toArg) {
	...
	if (to <= from) throw new RangeError("Invalid range: to must be greater than from");
...
function getMonthLabels(weeks, monthNames = DEFAULT_MONTH_LABELS) {
	return weeks.reduce((labels, week, weekIndex) => {
		...
		if (!firstActivity) throw new Error(`Unexpected error: Week ${weekIndex + 1} is empty.`);
		...
		if (weekIndex === 0 || prevLabel?.label !== month) return [...labels, {
			weekIndex,
			label: month
		}];
		return labels;
...
function maxWeekdayLabelWidth(labels, showWeekdayLabel, fontSize) {
	if (labels.length !== 7) throw new Error("Exactly 7 labels, one for each weekday must be passed.");
...
function calcTextDimensions(text, fontSize) {
	...
	if (fontSize < 1) throw new RangeError("fontSize must be positive");
...
function createTheme(input, levels = {
	...
	if (input) {
		validateThemeInput(input, levels.maxLevel - levels.minLevel + 1);
		input.light = input.light ?? defaultTheme.light;
		input.dark = input.dark ?? defaultTheme.dark;
		return {
			light: isPair(input.light) ? calcColorScale([
				input.light[1],
				input.light[0],
				input.light[1]
			], levels) : isTriple(input.light) ? calcColorScale(input.light, levels) : input.light,
			dark: isPair(input.dark) ? calcColorScale([
				input.dark[1],
				input.dark[0],
				input.dark[1]
			], levels) : isTriple(input.dark) ? calcColorScale(input.dark, levels) : input.dark
		};
	}
	return defaultTheme;
}
...
function validateThemeInput(input, numberOfLevels) {
	...
	if (typeof input !== "object" || input.light === void 0 && input.dark === void 0) throw new Error(`The theme object must contain at least one of the fields "light" and "dark" with exactly 2 or ${numberOfLevels} colors respectively. ${levelsHint}`);
	if (input.light) {
		const { length } = input.light;
		if (length !== 2 && length !== 3 && length !== numberOfLevels) throw new Error(`theme.light must contain exactly 2 or 3 or ${numberOfLevels} colors, ${length} passed. ${levelsHint}`);
		for (const c of input.light) if (typeof window !== "undefined" && !CSS.supports("color", c)) throw new Error(`Invalid color "${c}" passed. All CSS color formats are accepted.`);
	}
	if (input.dark) {
		const { length } = input.dark;
		if (length !== 2 && length !== 3 && length !== numberOfLevels) throw new Error(`theme.dark must contain exactly 2 or 3 or ${numberOfLevels} colors, ${length} passed. ${levelsHint}`);
		for (const c of input.dark) if (typeof window !== "undefined" && !CSS.supports("color", c)) throw new Error(`Invalid color "${c}" passed. All CSS color formats are accepted.`);
	}
}
function calcColorScale([colorNeg, colorZero, colorPos], { minLevel, maxLevel }) {
	return range(minLevel, maxLevel + 1).map((i) => {
		if (i < 0) {
			if (i === minLevel) return colorNeg;
			const pos = (1 - i / minLevel) * 100;
			return `color-mix(in oklab, ${colorZero} ${parseFloat(pos.toFixed(2))}%, ${colorNeg})`;
		}
		if (i === 0) return colorZero;
		if (i === maxLevel) return colorPos;
...
var styles = {
	...
	scrollContainer: (fontSize) => ({
		...
		overflowX: "auto",
		overflowY: "hidden",
		paddingTop: Math.ceil(.1 * fontSize)
	}),
	calendar: {
		display: "block",
		overflow: "visible"
	},
	rect: (colorScheme) => ({ stroke: colorScheme === "light" ? "rgba(0, 0, 0, 0.08)" : "rgba(255, 255, 255, 0.04)" }),
	footer: {
		container: {
			display: "flex",
			flexWrap: "wrap",
...
var Tooltip = /*#__PURE__*/ (0, import_react.lazy)(() => import("./Tooltip-B8gZJWmn-xwN7NaYP.js").then((module) => ({ default: module.Tooltip })));
var ActivityCalendar = /*#__PURE__*/ (0, import_react.forwardRef)(({ data: activities, blockMargin = 4, blockRadius = 2, blockSize = 12, className, colorScheme: colorSchemeProp, fontSize = 14, labels: labelsProp, loading = false, minLevel = 0, maxLevel = 4, renderBlock, renderColorLegend, showColorLegend = true, showMonthLabels = true, showTotalCount = true, showWeekdayLabels = false, style: styleProp = {}, theme: themeProp, tooltips = {}, weekStart = 0 }, ref) => {
	const [isClient, setIsClient] = (0, import_react.useState)(false);
	(0, import_react.useEffect)(() => {
		setIsClient(true);
	}, []);
	if (minLevel >= maxLevel) throw new RangeError(`Minimum activity level must be less than maximum level. Got ${minLevel} and ${maxLevel}.`);
	const levels = {
		minLevel,
		maxLevel
	};
	...
	if (loading) {
		if (useAnimation && !animationLoaded) return null;
	...
	const weekdayLabelOffset = isClient && weekdayLabels.shouldShow ? maxWeekdayLabelWidth(labels.weekdays, weekdayLabels, fontSize) + LABEL_MARGIN : void 0;
	...
	function renderCalendar() {
		return weeks.map((week, weekIndex) => week.map((activity, dayIndex) => {
			...
			const block = /*#__PURE__*/ (0, import_jsx_runtime.jsx)("rect", {
				x: 0,
			...
			return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(import_react.Fragment, { children: tooltips.activity ? /*#__PURE__*/ (0, import_jsx_runtime.jsx)(import_react.Suspense, {
			...
			}) : renderedBlock }, activity.date);
		})).map((week, x) => /*#__PURE__*/ (0, import_jsx_runtime.jsx)("g", {
			transform: `translate(${(blockSize + blockMargin) * x}, 0)`,
			children: week
		}, x));
	}
	function renderFooter() {
		if (!showTotalCount && !showColorLegend) return null;
		...
		return /*#__PURE__*/ (0, import_jsx_runtime.jsxs)("footer", {
			...
			children: [
				loading && /*#__PURE__*/ (0, import_jsx_runtime.jsx)("div", { children: "\xA0" }),
				!loading && showTotalCount && /*#__PURE__*/ (0, import_jsx_runtime.jsx)("div", {
				...
				!loading && showColorLegend && /*#__PURE__*/ (0, import_jsx_runtime.jsxs)("div", {
					className: getClassName("legend-colors"),
					style: styles.footer.legend,
					children: [
						labels.legend.less && /*#__PURE__*/ (0, import_jsx_runtime.jsx)("span", {
							style: { marginRight: "0.4em" },
							children: labels.legend.less
						}),
						range(minLevel, maxLevel + 1).map((level) => {
							const colorLegend = /*#__PURE__*/ (0, import_jsx_runtime.jsx)("svg", {
								width: blockSize,
								height: blockSize,
							...
							return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(import_react.Fragment, { children: tooltips.colorLegend ? /*#__PURE__*/ (0, import_jsx_runtime.jsx)(import_react.Suspense, {
							...
							}) : renderedColorLegend }, level);
						}),
						labels.legend.more && /*#__PURE__*/ (0, import_jsx_runtime.jsx)("span", {
	...
	function renderMonthLabels() {
		...
		return /*#__PURE__*/ (0, import_jsx_runtime.jsx)("g", {
			...
			children: getMonthLabels(weeks, labels.months).map(({ label, weekIndex }) => /*#__PURE__*/ (0, import_jsx_runtime.jsx)("text", {
...
//#endregion
export { getClassName as n, ActivityCalendar as t };
